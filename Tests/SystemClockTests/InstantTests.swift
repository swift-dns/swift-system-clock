...
struct InstantTests {
    @Test func `advancing and measuring are inverses`() {
        let start = SystemClock<Swift.Duration>.Instant.epoch.advanced(by: .seconds(10))
    ...
    @Test func `operators agree with the methods`() {
        let start = SystemClock<Swift.Duration>.Instant.epoch.advanced(by: .seconds(3))
    ...
    @Test func `ordering follows the reading`() {
        let earlier = SystemClock<Swift.Duration>.Instant.epoch
        let later = earlier.advanced(by: .nanoseconds(1))
        #expect(earlier < later)
        #expect(later > earlier)
        #expect(earlier != later)
        #expect(earlier == SystemClock<Swift.Duration>.Instant.epoch)
    }

    @Test func `equal instants hash equally`() {
        let first = SystemClock<Swift.Duration>.Instant.epoch.advanced(by: .seconds(7))
        let second = SystemClock<Swift.Duration>.Instant.epoch.advanced(by: .seconds(7))
        #expect(Set([first, second]).count == 1)
    }

    @Test func `an instant round-trips through Codable`() throws {
        let original = SystemClock<Swift.Duration>.Instant.epoch.advanced(
            by: .milliseconds(1_234_567)
        )
        let encoded = try JSONEncoder().encode(original)
        let decoded = try JSONDecoder().decode(
            SystemClock<Swift.Duration>.Instant.self,
            from: encoded
        )
    ...
    @Test func `a realtime reading keeps its full range`() {
        let reading = SystemClock<Swift.Duration>.realtime.now
        let components = SystemClock<Swift.Duration>.Instant.epoch.duration(to: reading).components
